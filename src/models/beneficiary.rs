...
impl Beneficiary {
    ...
    pub async fn can_claim(
        ...
        let row = sqlx::query_as::<_, Self>(
            "SELECT * FROM beneficiaries WHERE vault_id = $1 AND stellar_address = $2 AND claimed = FALSE",
        )
        .bind(vault_id)
    ...
    pub async fn record_claim(
        ...
        sqlx::query_as::<_, Self>(
            r#"UPDATE beneficiaries SET claimed = TRUE, claimed_at = NOW()
               WHERE vault_id = $1 AND stellar_address = $2 RETURNING *"#,
        )
        .bind(vault_id)
        .bind(stellar_address)
        .fetch_one(pool)
